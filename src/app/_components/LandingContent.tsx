  ...
  return (
    <>
      <main style={{ paddingTop: 56 }}>
        ...
        {/* Hero */}
        <section style={{ position: 'relative', padding: '72px 72px 96px', overflow: 'hidden', borderBottom: '1px solid rgba(40,40,44,0.5)' }}>
          <div style={{ position: 'absolute', inset: 0, opacity: 0.7, pointerEvents: 'none' }}>
            <MeshGraph width={1280} height={680} dense seed={3} />
          </div>
          <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse at 30% 50%, rgba(7,7,8,0) 0%, rgba(7,7,8,0.85) 60%, rgba(7,7,8,1) 100%)', pointerEvents: 'none' }} />
          <div style={{ position: 'relative', maxWidth: 760 }}>
            <span className="meta" style={{ color: '#a00020' }}>BLACKOUT · T+14:00 · LONDON</span>
            <h1 style={{ fontSize: 80, fontWeight: 700, lineHeight: 0.96, letterSpacing: '-0.03em', marginTop: 18, marginBottom: 0 }}>
              Trust is a<br />
              <span style={{ color: '#a00020' }}>network,</span><br />
              not a record.
            </h1>
            <p style={{ fontSize: 19, color: '#d2d2d6', marginTop: 28, maxWidth: 540, lineHeight: 1.55 }}>
            ...
            <div style={{ display: 'flex', gap: 12, marginTop: 36, alignItems: 'center' }}>
              <Link href="/register" style={{ padding: '14px 24px', background: '#a00020', color: '#f5f5f5', border: 'none', borderRadius: 8, fontWeight: 600, fontSize: 15, display: 'inline-flex', alignItems: 'center', gap: 8, textDecoration: 'none' }}>
        ...
        {/* How a node is born */}
        <section style={{ padding: '80px 72px', borderBottom: '1px solid rgba(40,40,44,0.5)' }}>
          ...
          <div style={{ position: 'relative', display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 0 }}>
            <svg style={{ position: 'absolute', top: 36, left: 0, right: 0, height: 2, width: '100%', pointerEvents: 'none' }} viewBox="0 0 1000 2" preserveAspectRatio="none">
              <line x1="60" y1="1" x2="940" y2="1" stroke="#a00020" strokeOpacity="0.4" strokeDasharray="4 6" />
            </svg>
            {[
              ['01', 'Identify', 'Name, password, one ID document.', '#a00020'],
              ['02', 'Evidence', 'Passport, degree, work ID.', '#a00020'],
              ['03', 'Vouch', 'People who know you confirm it.', '#a00020'],
              ['04', 'Verified', 'You appear on the public directory.', '#00b860'],
            ].map(([n, t, d, col]) => (
              <div key={n} style={{ paddingRight: 28 }}>
                ...
                <p style={{ fontSize: 14, color: '#d2d2d6', lineHeight: 1.55 }}>{d}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section style={{ padding: '80px 72px', borderBottom: '1px solid rgba(40,40,44,0.5)', background: 'rgba(18,18,20,0.35)' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 520px', gap: 48, alignItems: 'center' }}>
            <div>
              ...
              <p style={{ fontSize: 16, color: '#d2d2d6', marginBottom: 32, lineHeight: 1.55, maxWidth: 460 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                {[
                  ['Tier 0', 'Unverified', '0–19 pts', '#6a6a70'],
                  ['Tier 1', 'Verified', '20–54 pts', '#a00020'],
                  ['Tier 2', 'Trusted', '55–90 pts', '#a00020'],
                  ['Tier 3', 'Gov Official', '91–100 pts', '#00b860'],
                ].map(([t, n, p, col]) => (
                  <div key={t} style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '14px 18px', border: '1px solid rgba(40,40,44,0.5)', borderRadius: 10, background: '#121214' }}>
                    <div style={{ width: 8, height: 8, borderRadius: '50%', background: col, flexShrink: 0 }} />
                    <div style={{ width: 70 }}><span className="meta" style={{ color: col }}>{t}</span></div>
                    <div style={{ fontWeight: 600, flex: 1 }}>{n}</div>
                    <div className="mono" style={{ fontSize: 12, color: '#6a6a70' }}>{p}</div>
                  </div>
                ))}
              </div>
            </div>
            <div style={{ position: 'relative', height: 460, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <svg viewBox="0 0 460 460" width="100%" height="100%">
                {[200, 150, 100, 50].map((r, i) => {
                  const cols = ['#6a6a70', '#a00020', '#a00020', '#00b860']
                  return (
                    <circle
                ...
                {[
                  ...
                  [80, 100, '#6a6a70', 3], [380, 110, '#6a6a70', 3], [60, 320, '#6a6a70', 3], [400, 360, '#6a6a70', 3],
                  // Tier 1 dots — on ring r=150
                  [305, 100, '#a00020', 4], [155, 100, '#a00020', 4], [355, 310, '#a00020', 4], [105, 310, '#a00020', 4],
                  // Tier 2 dots — on ring r=100
                  [317, 180, '#a00020', 4], [143, 180, '#a00020', 4], [317, 280, '#a00020', 4], [143, 280, '#a00020', 4],
                ].map(([x, y, c, r], i) => (
                  <circle key={i} cx={x} cy={y} r={r} fill={c as string} />
                ))}
                <circle cx="230" cy="230" r="18" fill="#00b860" />
                <circle cx="230" cy="230" r="28" fill="none" stroke="#00b860" strokeOpacity="0.5" />
                <text x="230" y="234" textAnchor="middle" fontSize="9" fontWeight="700" letterSpacing="0.5" fill="#070708">GOV</text>
              </svg>
            </div>
          </div>
        </section>
        ...
        <section style={{ padding: '80px 72px', borderBottom: '1px solid rgba(40,40,44,0.5)' }}>
          ...
          <p style={{ color: '#d2d2d6', fontSize: 16, marginBottom: 48, maxWidth: 540 }}>Anti-fraud rules run on every claim and every vouch — without a central server.</p>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 18 }}>
            {[
              ...
              <div key={t as string} style={{ position: 'relative', overflow: 'hidden', border: '1px solid rgba(40,40,44,0.5)', borderRadius: 14, padding: 24, background: '#121214', height: 220 }}>
                ...
                <span className="material-symbols-outlined" style={{ fontSize: 22, color: '#a00020', display: 'block' }}>{icon}</span>
                <h4 style={{ fontSize: 18, fontWeight: 700, marginTop: 16, marginBottom: 8 }}>{t}</h4>
                <p style={{ fontSize: 13, color: '#d2d2d6', lineHeight: 1.55, maxWidth: 230 }}>{d}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section style={{ padding: '96px 72px', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ position: 'relative' }}>
            ...
            <p style={{ color: '#d2d2d6', fontSize: 16, maxWidth: 480, margin: '0 auto 32px' }}>Two minutes. One document. The rest happens as people around you vouch.</p>
            <Link href="/register" style={{ padding: '14px 28px', background: '#a00020', color: '#f5f5f5', border: 'none', borderRadius: 8, fontWeight: 600, fontSize: 15, display: 'inline-flex', alignItems: 'center', gap: 8, textDecoration: 'none' }}>
              <span className="material-symbols-outlined" style={{ fontSize: 18 }}>arrow_forward</span>
              Create your node
            </Link>
          </div>
        ...
        {/* Footer */}
        <footer style={{ padding: '20px 72px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid rgba(40,40,44,0.5)' }}>
          <span className="meta">CivicTrust · London Mesh · T+14:00:00</span>
          <div style={{ display: 'flex', gap: 18 }}>
            {['How it works', 'Aid hubs', 'Sign in'].map(l => (
              <Link key={l} href={l === 'Sign in' ? '/login' : '#'} style={{ fontSize: 12, color: '#6a6a70', textDecoration: 'none' }}>{l}</Link>
            ))}
          </div>
        </footer>
      </main>
