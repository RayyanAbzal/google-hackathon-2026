...
| P08 Resources without coordination | Yellow Pages + Post for Help matches supply/demand |
...
| 50–89 | Verified | Vouch others, post help, appear on map |
...
### Screen 5 — /help (Post for help)
- Post a request (skill or resource needed)
- Borough + urgency tag
- Visible to Verified users (50+) in that area
- Posts expire 24h
...
| 3 | QR vouch + Yellow Pages + Post for help | ~4h | Tao + Hemish + Maalav |
| 4 | Heatmap + map pins + counter + polish + demo | ~4h | Ray + Maalav + Hemish |

**Total: ~15h. Hackathon has 28h. Realistic.**
...
| Aryan | Backend | `src/app/api/` — register, login, claim, vouch, flag, score |
| Tao | Backend | `src/app/api/` (shared) — rate limiting, realtime, Yellow Pages API, post for help API |
| Hemish | Frontend | `src/components/` — score ring, profile card, forms, QR UI, polish |
| Maalav | Frontend/Pages | `src/app/` (non-API) — onboarding, profile page, map page, /find, /help |
...
- Resource claims / Information claims (post for help covers this)
- Score decay
- Voice input
- Offline / PWA
- P05 currency
